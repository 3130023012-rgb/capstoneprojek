@php
    ...
    // Variabel yang diharapkan dari Controller: $activity, $members, $memberStatuses
    $activity = $activity ?? null; 
    $members = $members ?? collect(); 
    $memberStatuses = $memberStatuses ?? []; // Array: [member_id => status]
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Update Absensi: {{ $activity->material ?? 'N/A' }} - UKM PAGARNUSA</title>
    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        select.status-select:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(0,66,37,0.1); }

        .btn-save { background: linear-gradient(135deg, var(--primary), var(--primary-2)); color: #fff; padding: 12px 25px; border-radius: 999px; border: none; font-weight: 900; cursor: pointer; transition: 0.2s; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 10px 20px rgba(0,66,37,0.15); }
        .btn-save:hover { transform: translateY(-2px); filter: brightness(1.1); }
        
        .btn-back { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; border-radius: 999px; font-weight: 800; text-decoration: none; background: rgba(100, 116, 139, 0.1); color: var(--muted); transition: 0.2s; }
        .btn-back:hover { background: rgba(100, 116, 139, 0.2); }
...
<body>
    ...
    <div class="content-wrapper">
        <div class="header-top">
            <h1><i class="fas fa-sync-alt" style="margin-right: 10px;"></i> Pembaruan Massal</h1>
        ...
        <div class="main-content-area">
            ...
            @if (session('error')) <div class="alert danger"><i class="fas fa-times-circle"></i> {{ session('error') }}</div> @endif

            <div class="page-header-box">
                <h2>Update Absensi Kegiatan</h2>
                <p style="margin-top:10px; font-weight: 700; color: var(--primary);">
                    <i class="fas fa-thumbtack"></i> Materi: {{ $activity->material ?? 'N/A' }} 
                    <span style="margin: 0 10px; color: var(--border);">|</span>
                    <i class="fas fa-calendar-alt"></i> {{ $activity ? Carbon::parse($activity->date)->translatedFormat('d F Y') : 'N/A' }}
                </p>
            </div>

            @if (!$activity)
                <div class="section-box" style="text-align: center;">
                    <p style="color: var(--color-danger); font-weight: 950;">Data kegiatan tidak ditemukan.</p>
                </div>
            @else
                <form action="{{ route('absensi.update_bulk', $activity->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="section-box">
                        <h3 style="margin: 0 0 20px; font-weight: 900; color: var(--primary); border-bottom: 2px solid var(--accent); display: inline-block; padding-bottom: 5px;">
                            ...
                            <table>
                                ...
                                <tbody>
                                    @forelse ($members as $member)
                                        @php
                                            $currentStatus = $memberStatuses[$member->id] ?? 'absent'; 
                                            $statusMap = [
                                                'present' => 'Hadir', 'absent' => 'Absen',
                                                'sick_leave' => 'Sakit', 'permission' => 'Izin',
                                            ];
                                            $statusText = $statusMap[$currentStatus] ?? 'Absen';

                                            $badgeClass = 'warning';
                                            if ($currentStatus == 'present') $badgeClass = 'success';
                                            if ($currentStatus == 'absent') $badgeClass = 'danger';
                                            if ($currentStatus == 'permission') $badgeClass = 'primary';
                                        @endphp
                                        <tr>
                                            ...
                                            <td>
                                                <select name="attendance[{{ $member->id }}]" class="status-select">
                                                    <option value="present" {{ $currentStatus == 'present' ? 'selected' : '' }}>Hadir</option>
                                                    <option value="absent" {{ $currentStatus == 'absent' ? 'selected' : '' }}>Absen</option>
                                                    <option value="sick_leave" {{ $currentStatus == 'sick_leave' ? 'selected' : '' }}>Sakit</option>
                                                    <option value="permission" {{ $currentStatus == 'permission' ? 'selected' : '' }}>Izin</option>
                                                </select>
                                                @error('attendance.' . $member->id)
                                                    <small style="color: var(--color-danger); display:block; margin-top:5px;">{{ $message }}</small>
                                                @enderror
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5" style="text-align:center; padding: 40px; color: var(--muted);">Data anggota tidak tersedia.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
                            <a href="{{ route('absensi.select') }}" class="btn-back">
                                <i class="fas fa-chevron-left"></i> Kembali ke Pilih Kegiatan
                            </a>
                            <button type="submit" class="btn-save">
                                <i class="fas fa-save"></i> Simpan Perubahan Massal
                            </button>
                        </div>
                    </div>
                </form>
