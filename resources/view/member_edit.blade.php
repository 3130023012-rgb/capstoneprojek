...
<head>
    ...
    <title>Edit Data Anggota: {{ $member->name }} - UKM PAGARNUSA</title>
    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        .main-container-form{ padding: 34px 26px; }

        .form-container{
            width: 100%; max-width: 980px; background: rgba(255,255,255,0.78);
            border: 1px solid rgba(15,23,42,0.06); border-radius: var(--radius);
            box-shadow: var(--shadow-md); padding: 22px; backdrop-filter: blur(6px);
        }

        .inner{ background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow-sm); padding: 18px; }

        .form-layout{ display:flex; gap: 22px; align-items:flex-start; flex-wrap: wrap; }
        .header-side{ flex: 1 1 280px; min-width: 260px; padding-top: 6px; }
        .form-fields-side{ flex: 2 1 420px; min-width: 320px; }

        .header-side h2{ margin: 0 0 6px; font-size: 1.6rem; font-weight: 950; color: var(--primary); letter-spacing:.15px; }
        ...
        .button-group{ display:flex; justify-content:flex-end; gap:10px; padding-top: 14px; margin-top: 14px; border-top: 1px solid rgba(0,0,0,.06); flex-wrap: wrap; }

        .btn{ padding: 10px 16px; border-radius: 999px; font-weight: 950; cursor:pointer; text-decoration:none; display:inline-flex; align-items:center; transition: all .12s ease; user-select:none; }
        .btn-simpan{ background: linear-gradient(135deg, var(--primary), var(--primary-2)); color: #fff; border: none; box-shadow: 0 14px 24px rgba(0,66,37,.18); }
...
<body>
    ...
    <div class="content-area">
        <div class="header-top-nav">
            ...
            <div style="font-weight: 800; color: var(--primary);"><i class="fas fa-user-edit"></i> Edit Access</div>
        </div>
                    ...
                    <form action="{{ route('member.update', $member) }}" method="POST">
                        ...
                        <div class="form-layout">
                            <div class="header-side">
                                <h2>Edit Profil</h2>
                                <p>Perbarui data biodata anggota <strong>{{ $member->name }}</strong> agar tetap akurat di sistem.</p>
                            </div>
                            ...
                            <div class="form-fields-side">
                                ...
                                <div class="form-group">
                                    <label for="member_id">NIM / NIA</label>
                                    <input type="text" name="member_id" id="member_id"
                                           value="{{ old('member_id', $member->member_id) }}"
                                           placeholder="Masukkan NIM / NIA">
                        ...
                        <div class="button-group">
                            <a href="{{ route('member.index', $member) }}" class="btn btn-batal">Batal</a>
