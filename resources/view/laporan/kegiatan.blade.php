@php
    ...
    $kasTransactions = $kasTransactions ?? collect();
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Laporan Kegiatan - UKM PAGARNUSA</title>
    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        th{ text-align: left; padding: 15px; background: #f8fafc; color: var(--primary); font-weight: 900; border: 1px solid var(--border); font-size: 0.9em; }
        td{ padding: 15px; border: 1px solid var(--border); font-weight: 700; font-size: 0.95em; }
        
        .nominal-col { text-align: right; font-weight: 800; color: var(--primary); white-space: nowrap; font-family: 'Courier New', Courier, monospace; }
...
<body>
        ...
        <li>
            <details open>
                ...
                <ul class="sidebar-dropdown">
                    ...
                    <li><a href="{{ route('laporan.kegiatan') }}" class="active">Kegiatan</a></li>
                </ul>
            </details>
        </li>
    ...
    <div class="content">
        <div class="header-top">
            <h1><i class="fas fa-file-signature" style="margin-right: 10px;"></i> Laporan Kegiatan</h1>
            ...
            <div class="report-card">
                <div class="filter-row">
                    <form method="GET" action="{{ route('laporan.kegiatan') }}">
                        ...
                        <button type="submit" style="background: var(--primary); color: white; border: none; padding: 10px 20px; border-radius: 12px; font-weight: 800; cursor: pointer; margin-left: 5px;"><i class="fas fa-filter"></i> Filter</button>
                    </form>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th style="width: 40%;">Materi / Nama Kegiatan</th>
                                <th style="width: 20%;">Pelatih / PJ</th>
                                <th style="width: 25%; text-align: right;">Anggaran (Nominal)</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($activities as $act)
                            <tr>
                                <td>{{ \Carbon\Carbon::parse($act->date)->translatedFormat('d M Y') }}</td>
                                <td style="color: var(--primary);">{{ $act->material }}</td>
                                <td>{{ $act->trainer->name ?? '-' }}</td>
                                <td class="nominal-col">
                                    Rp {{ number_format($act->nominal_kas, 0, ',', '.') }}
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" style="text-align:center; padding: 40px; color: var(--muted);">
                                    <i class="fas fa-calendar-times fa-2x" style="display: block; margin-bottom: 10px; opacity: 0.3;"></i>
                                    Tidak ada data pelaksanaan kegiatan pada periode ini.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
